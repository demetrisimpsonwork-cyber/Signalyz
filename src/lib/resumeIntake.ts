...
function extractExperienceBlocks(lines: string[], isProjects: boolean): ExtractedExperience[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Responsibility line — reject contact info, header artifacts, and non-experience debris from bullets
    if (current) {
      if (isContactInfoLine(line) || isPhoneOrEmail(line)) continue;
      if (/^[A-Z]{15,}$/.test(line.trim())) continue;
      if (isEducationOrLocationDebris(line)) continue;
      if (isResponsibilityLine(lines[i])) {
        current.responsibilities.push(line);
      } else if (line.length > 15 && !isEducationOrLocationDebris(line)) {
        current.responsibilities.push(line);
      }
    } else if (isResponsibilityLine(lines[i]) && line.length > 15 && !isContactInfoLine(line) && !isPhoneOrEmail(line) && !isEducationOrLocationDebris(line)) {
