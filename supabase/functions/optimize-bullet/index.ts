...
JSON SCHEMA:
{
  ...
  "signal_verdict": {"current_positioning":"string — one direct sentence describing how the candidate currently reads to hiring managers based on their resume signals. No hedging. No 'may' or 'might'. State the impression as fact. Example: 'You read as a mid-level project coordinator who tracks deliverables, not a program leader who owns outcomes.'","target_expectation":"string — one direct sentence describing what the role actually requires at the signal level. What does a competitive candidate for THIS role look like on paper? Example: 'This role requires evidence of cross-functional ownership, commercial impact, and stakeholder influence at the director level.'","core_gap":"string — one direct sentence identifying the single most likely reason the candidate is being filtered or not selected. This is NOT the same as primary_blocker — it is a higher-level PERCEPTION judgment, not a tactical gap. Choose the strongest interpretation and state it clearly. No lists, no multiple issues, no fixes, no reasoning. Example: 'You are being filtered because your resume signals execution support, not strategic ownership.'"} (JUDGMENT LAYER — NOT AN EXPLANATION LAYER. Do NOT summarize or repeat primary_blocker, what_hiring_managers_see, or any other section. Do NOT list multiple issues. Do NOT include fixes or recommendations. This is the hiring manager's immediate impression — decisive, confident, singular.),
