...
JSON SCHEMA:
{
  ...
  "signal_verdict": {"current_positioning":"string — 8-16 words. The hiring manager's snap read. State the identity the resume projects as fact. No 'appears to' or 'seems like'. Pattern: 'You read as [identity], not [expected identity].' Example: 'You read as a process coordinator, not a growth operator.'","target_expectation":"string — 8-16 words. What a winning candidate signals on paper for THIS role. No 'this role requires evidence of'. Pattern: 'Winning candidates signal [X], [Y], and [Z].' Example: 'Winning candidates signal P&L ownership, cross-functional authority, and commercial outcomes.'","core_gap":"string — 8-16 words. The single perception failure causing rejection. Not a skill gap — a SIGNAL gap. Must be distinct from primary_blocker (verdict = perception; blocker = tactical cause). Pattern: 'Your resume signals [wrong thing] instead of [right thing].' Example: 'Your resume signals task management instead of business ownership.'"} (PERCEPTION SNAPSHOT — not analysis. Three sharp judgments a hiring manager forms in 10 seconds. No explanations, no fixes, no lists, no hedging. Must feel like an internal note, not a report. Must NOT overlap with primary_blocker content.),
